...
describe('browser.tinymce.plugins.table.NestedFakeSelectionTest', () => {
  ...
  // The critical part is the target as this is what Darwin (MouseSelection.ts) uses to determine the fake selection
  const selectWithMouse = (editor: Editor, start: SugarElement<Element>, end: SugarElement<Element>) => {
    editor.fire('mousedown', { target: start.dom, button: 0 } as unknown as MouseEvent);
    editor.fire('mouseover', { target: end.dom, button: 0 } as unknown as MouseEvent);
    editor.fire('mouseup', { target: end.dom, button: 0 } as unknown as MouseEvent);
  };
  ...
  const assertMouseTableSelection = (editor: Editor, content: string, selection: Cursors.CursorPath, expectedSelectedCells: string[]) => {
    ...
    selectWithMouse(editor, cursorRange.start, cursorRange.finish);
  ...
  it('TINY-6298: Partially selecting cell content in the nested table', () => {
    ...
    assertTableSelection(
      editor,
      {
        content,
        selection: {
          startPath: [ ...firstTableCell, 1, ...firstTableCell ],
          soffset: 0,
          finishPath: [ ...firstTableCell, 1, ...firstTableCell ],
          foffset: 3,
        },
  ...
  it('TINY-6298: Partially selecting cell content in outer table that include nested table in selection', () => {
    const editor = hook.editor();
    const content = (
      '<table>' +
      '<tbody>' +
  ...
  it('TINY-6298: Selecting cells in the nested table', () => {
    ...
    const firstTableRow = [ 0, 0, 0 ];
    assertTableSelection(
      editor,
      {
        content,
        selection: {
          startPath: [ ...firstTableRow, 0, 1, ...firstTableRow, 0 ],
          soffset: 0,
          finishPath: [ ...firstTableRow, 0, 1, 0, 0, 1, 0 ],
          foffset: 3,
        },
  ...
  it('TINY-6298: Selecting from nested table cell to the parent outer table cell', () => {
    ...
    assertTableSelection(
      editor,
      {
        content,
        selection: {
          startPath: [ ...firstTableRow, 0, 1, 0, 0, 0, 0 ],
          soffset: 1,
          finishPath: [ ...firstTableRow, 0, 2, 0 ],
          foffset: 1,
        },
        expectedSelectedCells: [],
        keyDirection: Keys.right()
